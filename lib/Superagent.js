...
function decodeResError(res, metadata) {
...
const Request = (function() {
  ...
  SuperagentRequest.prototype.redirects = function(object) {};
  ...
  SuperagentRequest.prototype.end = async function(callback) {
    ...
    const context = await this.agent._execute({ request });
    const { res, metadata, httpResponse, errorPassedToNext } = context;

    decorateResponse(res, httpResponse);

    if (callback) {
      callback(decodeResError(res, metadata), res);
